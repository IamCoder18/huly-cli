...
export async function getComponent(
  ...
  try {
    ...
    header(`Component — ${doc.label ?? '(unnamed)'}`, {
...
export async function createComponent(opts: {
  ...
  try {
    ...
    const id = await withSpinner(
      ...
      () =>
        client.createDoc(
          CLASS.Component as Ref<Class<Component>>,
...
export async function updateComponent(
  ...
  try {
    ...
    if (Object.keys(ops).length === 0)
      throw new CliError(ExitCode.Validation, 'nothing to update', 'pass --label or --description')
    if (opts.dryRun) {
...
export async function deleteComponents(
  ...
  try {
    ...
    if (!opts.yes && ids.length > 1)
      throw new CliError(
        ExitCode.Validation,
        `destructive: deleting ${ids.length} components requires --yes`,
        're-run with --yes to confirm',
      )
    let deleted = 0,
      skipped = 0
    for (const id of ids) {
      const doc = await client.findOne(CLASS.Component as Ref<Class<Component>>, {
        _id: id as Ref<Component>,
      })
      if (!doc) {
        skipped++
        continue
