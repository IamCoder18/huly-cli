    ...
    table(
      ...
      [
        { key: 'label', header: 'LABEL' },
        {
          ...
          header: 'DISABLE',
          align: 'center',
          format: (r) => ((r as NotificationProvider).canDisable ? C.muted('yes') : C.warn('no')),
        },
        {
          key: '_id',
          header: '_ID',
          format: (r) => C.id(String((r as NotificationProvider)._id).slice(-16)),
        },
      ],
    ...
    table(
      ...
      [
        { key: 'label', header: 'LABEL' },
        {
          key: 'group',
          header: 'GROUP',
          format: (r) => C.id(String((r as NotificationType).group).slice(-12)),
        },
        {
          key: 'objectClass',
          header: 'OBJECT',
          format: (r) =>
            String((r as NotificationType).objectClass)
              .split(':')
              .pop() ?? '',
        },
        {
          ...
          header: 'ENABLED',
          align: 'center',
          format: (r) => ((r as NotificationType).defaultEnabled ? C.ok('✓') : C.muted('—')),
        },
...
export async function listInbox(opts: ListInboxOpts = {}): Promise<void> {
  ...
  try {
    ...
    const docs = (await withSpinner(
      ...
      () => client.findAll(INBOX_CLASS, q as any),
      opts,
    ...
    if (r.length === 0) {
      console.log(C.muted('(no notifications)'))
      return
    }
    table(
      ...
      [
        {
          ...
          header: 'KIND',
          format: (r) =>
            String((r as InboxNotification).objectClass)
              .split(':')
              .pop() ?? '',
        },
        {
          ...
          header: 'READ',
          align: 'center',
          format: (r) => ((r as InboxNotification).isViewed ? C.muted('—') : C.warn('●')),
        },
        {
          key: 'archived',
          header: 'ARC',
          align: 'center',
          format: (r) => ((r as InboxNotification).archived ? C.muted('✓') : C.muted('—')),
        },
        {
          key: 'objectId',
          header: 'OBJECT',
          format: (r) => C.id(String((r as InboxNotification).objectId).slice(-12)),
        },
        {
          key: 'modifiedOn',
          header: 'WHEN',
          align: 'right',
          format: (r) => relTime((r as InboxNotification).modifiedOn as number | null),
        },
...
async function updateInbox(
  ...
  const id = (await resolveRef(ref, {
    client,
    classId: INBOX_CLASS as Ref<Class<Doc>>,
  })) as Ref<InboxNotification>
  const doc = await client.findOne(INBOX_CLASS, { _id: id })
  if (!doc) throw new CliError(ExitCode.NotFound, `notification ${ref} not found`)
  await withSpinner(
    spinner,
    () => client.updateDoc(INBOX_CLASS, doc.space as Ref<Doc>, id as Ref<Doc>, ops as any),
    opts,
  )
  return id
}
...
export async function markRead(
  refs: string[],
  opts: { workspace?: string; url?: string; json?: boolean; ci?: boolean; yes?: boolean } = {},
): Promise<void> {
  const client = await connectCli({ url: opts.url, workspace: opts.workspace })
  try {
    if (!opts.yes && refs.length > 1)
      throw new CliError(
        ExitCode.Validation,
        `destructive: marking ${refs.length} notifications read requires --yes`,
      )
    let count = 0
    for (const r of refs) {
...
export async function markUnread(
  ...
  try {
    if (!opts.yes && refs.length > 1)
      throw new CliError(
        ExitCode.Validation,
        `destructive: marking ${refs.length} notifications unread requires --yes`,
      )
    let count = 0
    for (const r of refs) {
...
export async function markAllRead(
  ...
  try {
    const account = await client.getAccount()
    const docs = (await client.findAll(INBOX_CLASS, {
      user: account.uuid,
      isViewed: false,
    })) as InboxNotification[]
    if (docs.length === 0) {
      console.log(C.muted('(no unread notifications)'))
      return
    }
    for (const d of docs) {
...
export async function archive(
  ...
  try {
    if (!opts.yes && refs.length > 1)
      throw new CliError(
        ExitCode.Validation,
        `destructive: archiving ${refs.length} notifications requires --yes`,
      )
    let count = 0
    for (const r of refs) {
...
export async function unarchive(
  ...
  try {
    if (!opts.yes && refs.length > 1)
      throw new CliError(
        ExitCode.Validation,
        `destructive: unarchiving ${refs.length} notifications requires --yes`,
      )
    let count = 0
    for (const r of refs) {
...
export async function deleteInbox(
  ...
  try {
    ...
    if (!opts.yes && ids.length > 1)
      throw new CliError(
        ExitCode.Validation,
        `destructive: deleting ${ids.length} notifications requires --yes`,
      )
    let deleted = 0,
      skipped = 0
    for (const id of ids) {
...
export async function unreadCount(
  opts: { workspace?: string; url?: string; json?: boolean } = {},
): Promise<void> {
  const client = await connectCli({ url: opts.url, workspace: opts.workspace })
  try {
    const account = await client.getAccount()
...
export async function listContexts(opts: ListContextsOpts = {}): Promise<void> {
  ...
  try {
    ...
    const docs = (await withSpinner(
      'Loading contexts…',
      () => client.findAll(CONTEXT_CLASS, q as any),
      opts,
    )) as DocNotifyContext[]
    if (shouldJson({ json: opts.json, ci: opts.ci })) {
      json(docs)
    ...
    if (docs.length === 0) {
      console.log(C.muted('(no contexts)'))
      return
    }
    table(
      docs as unknown as Record<string, unknown>[],
      [
        {
          key: 'objectId',
          header: 'OBJECT',
          format: (r) => C.id(String((r as DocNotifyContext).objectId).slice(-12)),
        },
        {
          key: 'objectClass',
          header: 'CLASS',
          format: (r) =>
            String((r as DocNotifyContext).objectClass)
              .split(':')
              .pop() ?? '',
        },
        {
          ...
          header: 'PIN',
          align: 'center',
          format: (r) => ((r as DocNotifyContext).isPinned ? C.warn('★') : C.muted('—')),
        },
        {
          key: 'hidden',
          header: 'HIDDEN',
          align: 'center',
          format: (r) => ((r as DocNotifyContext).hidden ? C.muted('yes') : C.muted('—')),
        },
...
export async function pinContext(
  ref: string,
  opts: { unpin?: boolean; workspace?: string; url?: string } = {},
): Promise<void> {
  const client = await connectCli({ url: opts.url, workspace: opts.workspace })
  try {
    const id = (await resolveRef(ref, {
      client,
      classId: CONTEXT_CLASS as Ref<Class<Doc>>,
    })) as Ref<DocNotifyContext>
...
export async function hideContext(
  ref: string,
  opts: { unhide?: boolean; workspace?: string; url?: string } = {},
): Promise<void> {
  const client = await connectCli({ url: opts.url, workspace: opts.workspace })
  try {
    const id = (await resolveRef(ref, {
...
export async function unsubscribe(opts: SubOpts): Promise<void> {
  ...
  try {
    ...
    const tId = await resolveRef(opts.target, { client, classId: tClass })
    const account = await client.getAccount()
    const docs = (await client.findAll(CONTEXT_CLASS, {
      user: account.uuid,
      objectId: tId,
      objectClass: tClass,
    })) as DocNotifyContext[]
    if (docs.length === 0) {
      console.log(C.muted('(not subscribed)'))
      return
    }
    for (const d of docs) {
...
export async function listSettings(opts: SettingsOpts = {}): Promise<void> {
  ...
  try {
    ...
    if (docs.length === 0) {
      console.log(C.muted('(no settings)'))
      return
    }
    table(
      docs as unknown as Record<string, unknown>[],
      [
        {
          key: 'attachedTo',
          header: 'PROVIDER',
          format: (r) => C.id(String((r as NotificationTypeSetting).attachedTo).slice(-12)),
        },
        {
          key: 'type',
          header: 'TYPE',
          format: (r) => C.id(String((r as NotificationTypeSetting).type).slice(-12)),
        },
        {
          key: 'enabled',
          header: 'ENABLED',
          align: 'center',
          format: (r) => ((r as NotificationTypeSetting).enabled ? C.ok('✓') : C.warn('—')),
        },
      ],
...
export async function updateSetting(opts: UpdateSettingOpts): Promise<void> {
  ...
  try {
    ...
    const existing = (await client.findAll(TYPE_SETTING_CLASS, {
      attachedTo: opts.provider as Ref<Doc>,
      type: opts.type as Ref<Doc>,
      modifiedBy: String(account.uuid),
    })) as NotificationTypeSetting[]
    if (existing.length > 0) {
      await client.updateDoc(
        TYPE_SETTING_CLASS,
        existing[0].space as Ref<Doc>,
        existing[0]._id as Ref<Doc>,
        { enabled: opts.enabled } as any,
      )
      updated('updated setting', refString(existing[0]._id))
    } else {
      const id = await client.addCollection(
        TYPE_SETTING_CLASS,
        account.person as Ref<Doc>,
        opts.provider as Ref<Doc>,
        PROVIDER_CLASS,
        'types',
        {
          type: opts.type as Ref<Doc>,
