...
export async function getIssueTemplate(
  ref: string,
  opts: { json?: boolean; ci?: boolean; markdown?: boolean; workspace?: string; url?: string } = {},
): Promise<void> {
  const client = await connectCli({ url: opts.url, workspace: opts.workspace })
  try {
    ...
    const doc = await client.findOne(CLASS.IssueTemplate as Ref<Class<IssueTemplate>>, {
      _id: id as Ref<IssueTemplate>,
    })
    ...
    if (opts.markdown && doc.description) {
      try {
        const body = await withTimeout(
          client.fetchMarkup(
            CLASS.IssueTemplate as Ref<Class<Doc>>,
            doc._id,
            'description',
    ...
    header(`Issue template — ${doc.title ?? '(untitled)'}`, {
...
export async function createIssueTemplate(opts: {
  ...
  try {
    ...
    const id = await withSpinner(
      ...
      () =>
        client.createDoc(
          CLASS.IssueTemplate as Ref<Class<IssueTemplate>>,
...
export async function updateIssueTemplate(
  ...
  try {
    ...
    if (opts.dryRun) {
      ...
      console.log(
        JSON.stringify({ _class: CLASS.IssueTemplate, objectId: id, space: doc.space, ops }, null, 2),
      )
      return
    }
    await withSpinner(
...
export async function deleteIssueTemplates(
  refs: string[],
  ...
  try {
    ...
    if (!opts.yes && ids.length > 1)
      throw new CliError(
        ExitCode.Validation,
        `destructive: deleting ${ids.length} templates requires --yes`,
    ...
    await withSpinner(
      ...
      () =>
        client.updateDoc(
          CLASS.IssueTemplate as Ref<Class<IssueTemplate>>,
          doc.space as unknown as Ref<Space>,
          templateId as Ref<IssueTemplate>,
          { children } as any,
        ),
      opts,
    )
...
export async function removeTemplateChild(
  templateRef: string,
  childRef: string,
  opts: { json?: boolean; ci?: boolean; dryRun?: boolean; workspace?: string; url?: string },
): Promise<void> {
  ...
  try {
    ...
    const children = (doc.children ?? []).filter(
      (c: Record<string, unknown>) => (c as { id?: string }).id !== childId,
    )
    if (opts.dryRun) {
