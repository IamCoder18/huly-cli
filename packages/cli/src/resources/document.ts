...
async function resolveTeamspace(client: PlatformClient, ref?: string): Promise<Teamspace> {
  ...
  if (all.length === 0) {
    ...
    const id = await client.createDoc(TEAMSPACE_CLASS, TEAMSPACE_DEFAULT, {
      name: 'General',
...
async function resolveDocumentByTitle(
  ...
  if (hits.length > 1) {
    throw new CliError(
      ExitCode.Ambiguous,
      `multiple documents titled "${title}" — pass a _id ref instead`,
      `matches: ${hits.map((d) => d._id).join(', ')}`,
    )
  }
  return hits[0]
}
    ...
    table(
      ...
      [
        { key: 'name', header: 'NAME' },
        {
          key: 'description',
          header: 'DESCRIPTION',
          format: (r) => String((r as Teamspace).description ?? '').slice(0, 60),
        },
        { key: 'type', header: 'TYPE' },
        {
          ...
          header: 'PRIVATE',
          format: (r) =>
            r != null ? ((r as Teamspace).private ? C.red('private') : C.green('shared')) : C.muted('—'),
        },
        {
          key: 'archived',
          header: 'STATE',
          format: (r) =>
            r != null ? ((r as Teamspace).archived ? C.red('archived') : C.green('active')) : C.muted('—'),
        },
  ...
  try {
    ...
    if (Object.keys(ops).length === 0)
      throw new CliError(
        ExitCode.Validation,
        'nothing to update',
        'pass --name, --description, or --archived',
      )
    if (opts.dryRun) {
...
export async function deleteTeamspaces(
  refs: string[],
  ...
  try {
    ...
    if (!opts.yes && ids.length > 1)
      throw new CliError(
        ExitCode.Validation,
        `destructive: deleting ${ids.length} teamspaces requires --yes`,
...
export async function listDocuments(
  ...
  try {
    ...
    const docs = (await withSpinner(
      'Loading documents…',
      () => client.findAll(DOCUMENT_CLASS, query as any),
      opts,
    )) as Document[]
    let r = docs
    if (opts.offset && opts.offset > 0) r = r.slice(opts.offset)
...
export async function getDocument(
  ...
  try {
    ...
    if ((opts.markdown || opts.rawMarkup) && doc.content) {
      try {
        const body = await withTimeout(
          client.fetchMarkup(
            DOCUMENT_CLASS as Ref<Class<Doc>>,
            doc._id,
            'content',
            doc.content as any,
    ...
    header(`Document — ${doc.title ?? '(untitled)'}`, {
      subtitle: `created ${relTime(doc.createdOn as number | null)} · updated ${relTime(doc.modifiedOn as number | null)}`,
    })
    kv([
  ...
  try {
    ...
    if (opts.parent) {
      try {
        ...
        // try by title
        const found = await resolveDocumentByTitle(
          client,
          teamspace._id as unknown as Ref<Teamspace>,
          opts.parent,
        )
        parent = found._id
      }
    ...
    if (opts.dryRun) {
      ...
      console.log(
        JSON.stringify({ _class: DOCUMENT_CLASS, _id: newDocId, space: teamspace._id, data }, null, 2),
      )
      return
    }
    const id = await withSpinner(
  ...
  try {
    ...
    if (body !== undefined) {
      if (opts.oldText && opts.newText !== undefined) {
        throw new CliError(
          ExitCode.Validation,
          'ambiguous: pass either --body (full replace) OR --old-text/--new-text (targeted), not both',
        )
      }
      ...
      if (occurrences === 0) {
        throw new CliError(
          ExitCode.NotFound,
          `old-text not found in document`,
          'pass --dry-run to inspect, or check the source body',
        )
      }
    ...
    if (Object.keys(ops).length === 0 && !markupUpdated) {
      throw new CliError(
        ExitCode.Validation,
        'nothing to update',
        'pass --body, --title, --archived, or --old-text/--new-text',
      )
    }
    if (opts.dryRun) {
      ...
      console.log(
        JSON.stringify(
          { _class: DOCUMENT_CLASS, objectId: id, space: doc.space, ops, markupUpdated },
          null,
          2,
        ),
      )
...
export async function deleteDocuments(
  ...
  try {
    ...
    if (!opts.yes && ids.length > 1)
      throw new CliError(
        ExitCode.Validation,
        `destructive: deleting ${ids.length} documents requires --yes`,
        're-run with --yes to confirm',
      )
    let deleted = 0,
      skipped = 0
      ...
      [
        {
          key: 'title',
          header: 'TITLE',
          format: (r) => C.emphasis(String((r as DocumentSnapshot).title ?? '')),
        },
        {
          key: 'createdOn',
          header: 'CREATED',
          format: (r) => (r != null ? relTime((r as DocumentSnapshot).createdOn) : C.muted('—')),
        },
  ...
  try {
    ...
    if ((opts.markdown || opts.rawMarkup) && snap.content) {
      try {
        const body = await withTimeout(
          client.fetchMarkup(
            SNAPSHOT_CLASS as Ref<Class<Doc>>,
            snap._id,
            'content',
            snap.content as any,
            opts.rawMarkup ? 'markup' : 'markdown',
        ...
        console.log(String(snap.content))
        return
      }
    }
    if (shouldJson({ json: opts.json, ci: opts.ci })) {
    ...
    header(`Snapshot — ${snap.title ?? '(untitled)'}`, {
      subtitle: `created ${relTime(snap.createdOn as number | null)}`,
    })
    kv([
      ...
      [
        'Created',
        snap.createdOn != null
          ? `${isoDate(snap.createdOn)} (${relTime(snap.createdOn as number | null)})`
          : C.muted('—'),
      ],
    ])
...
export async function listInlineComments(
  ...
  try {
    ...
    const comments = (await client.findAll('chunter:class:ChatMessage' as Ref<Class<InlineComment>>, {
      attachedTo: id as Ref<Doc>,
      attachedToClass: DOCUMENT_CLASS,
      collection: 'comments',
    })) as InlineComment[]
    if (shouldJson({ json: opts.json, ci: opts.ci })) {
      json(comments)
