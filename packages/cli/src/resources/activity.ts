...
export async function listActivity(opts: ListActivityOpts = {}): Promise<void> {
  ...
  try {
    ...
    const docs = (await withSpinner(
      'Loading activity…',
      () => client.findAll(ACTIVITY_CLASS, q as any),
      opts,
    )) as ActivityMessage[]
    let r = docs
    ...
    if (r.length === 0) {
      console.log(C.muted('(no activity)'))
      return
    }
    table(
      ...
      [
        {
          ...
          header: 'MESSAGE',
          format: (r) => {
            const m = String((r as ActivityMessage).message ?? '')
              .replace(/\n/g, ' ')
              .slice(0, 60)
        ...
        },
        {
          ...
          header: 'PIN',
          align: 'center',
          format: (r) => ((r as ActivityMessage).isPinned ? C.warn('★') : C.muted('—')),
        },
        {
          key: 'attachedTo',
          header: 'TARGET',
          format: (r) => C.id(String((r as ActivityMessage).attachedTo ?? '').slice(-12)),
        },
        {
          key: 'createdOn',
          header: 'WHEN',
          align: 'right',
          format: (r) => relTime((r as ActivityMessage).createdOn as number | null),
        },
...
export async function pinActivity(
  ref: string,
  opts: { unpin?: boolean; workspace?: string; url?: string; json?: boolean; ci?: boolean } = {},
): Promise<void> {
  const client = await connectCli({ url: opts.url, workspace: opts.workspace })
  try {
    const id = await resolveRef(ref, { client, classId: ACTIVITY_CLASS as Ref<Class<Doc>> })
...
async function fetchActivity(
  ...
  ref: string,
): Promise<{ id: Ref<ActivityMessage>; doc: ActivityMessage }> {
  const id = (await resolveRef(ref, {
    client,
    classId: ACTIVITY_CLASS as Ref<Class<Doc>>,
  })) as Ref<ActivityMessage>
...
export async function removeReaction(opts: ReactionOpts): Promise<void> {
  ...
  try {
    ...
    const account = await client.getAccount()
    const reactions = (await client.findAll(REACTION_CLASS, {
      attachedTo: id,
      emoji: opts.emoji,
      createBy: account.primarySocialId,
    })) as Reaction[]
    if (reactions.length === 0) {
      console.log(C.muted('(no matching reaction)'))
      return
    }
...
export async function listReactions(
  target: string,
  opts: { workspace?: string; url?: string; json?: boolean; ci?: boolean } = {},
): Promise<void> {
  const client = await connectCli({ url: opts.url, workspace: opts.workspace })
  try {
    ...
    if (reactions.length === 0) {
      console.log(C.muted('(no reactions)'))
      return
    }
    table(
...
export async function listReplies(
  ...
  try {
    const { id } = await fetchActivity(client, target)
    const replies = (await client.findAll(ACTIVITY_CLASS, {
      attachedTo: id,
      attachedToClass: ACTIVITY_CLASS,
    })) as ActivityMessage[]
    if (shouldJson({ json: opts.json, ci: opts.ci })) {
      json(replies)
      return
    ...
    if (replies.length === 0) {
      console.log(C.muted('(no replies)'))
      return
    }
    table(
      replies as unknown as Record<string, unknown>[],
      [
        {
          key: 'message',
          header: 'REPLY',
          format: (r) =>
            String((r as ActivityMessage).message ?? '')
              .replace(/\n/g, ' ')
              .slice(0, 60) || C.muted('(empty)'),
        },
...
export async function updateReply(
  ref: string,
  opts: { body?: string; workspace?: string; url?: string } = {},
): Promise<void> {
...
export async function deleteReplies(
  ...
  try {
    ...
    if (!opts.yes && ids.length > 1)
      throw new CliError(ExitCode.Validation, `destructive: deleting ${refs.length} replies requires --yes`)
    const removed: Ref<Doc>[] = []
    const errors: Array<{ id: string; message: string }> = []
    for (const id of ids) {
      const doc = (await client.findOne(ACTIVITY_CLASS, { _id: id as Ref<ActivityMessage> })) as
        | ActivityMessage
        | undefined
      if (!doc || !doc.attachedTo) {
        errors.push({ id: String(id), message: 'not found or unattached' })
        continue
      }
      try {
        await client.removeCollection(
          ...
          id as Ref<Doc>,
          doc.attachedTo,
          doc.attachedToClass ?? ACTIVITY_CLASS,
          doc.collection ?? 'replies',
        )
...
export async function listSaved(opts: SavedOpts = {}): Promise<void> {
  ...
  try {
    ...
    const account = await client.getAccount()
    const docs = (await client.findAll(SAVED_CLASS, {
      space: SPACE.Workspace as Ref<Doc>,
    ...
    if (docs.length === 0) {
      console.log(C.muted('(no saved messages)'))
      return
    }
    table(
      ...
      [
        {
          key: 'attachedTo',
          header: 'MESSAGE',
          format: (r) => C.id(String((r as SavedMessage).attachedTo).slice(-12)),
        },
...
export async function saveMessage(opts: SavedOpts): Promise<void> {
  ...
  try {
    ...
    const sid = await withSpinner(
      'Saving…',
      () => client.createDoc(SAVED_CLASS, SPACE.Workspace as Ref<Space>, data as any),
      opts,
    )
...
export async function unsaveMessage(opts: SavedOpts): Promise<void> {
  ...
  try {
    ...
    const account = await client.getAccount()
    const all = (await client.findAll(SAVED_CLASS, {
      space: SPACE.Workspace as Ref<Doc>,
      attachedTo: id,
      modifiedBy: String(account.uuid),
    })) as SavedMessage[]
...
export async function listMentions(
  ...
  try {
    ...
    if (account.person === undefined) {
      const byUuid = (await client.findAll(MENTION_CLASS, {
        user: account.uuid as unknown as Ref<Doc>,
      })) as UserMentionInfo[]
      if (shouldJson({ json: opts.json, ci: opts.ci })) {
        json(byUuid)
        return
    ...
    const me = await client.findOne('contact:class:Person' as Ref<Class<Doc>>, {
      _id: account.person as unknown as Ref<Doc>,
    })
...
function renderMentions(all: UserMentionInfo[]): void {
  if (all.length === 0) {
    console.log(C.muted('(no mentions)'))
    return
  }
  table(
    ...
    [
      {
        key: 'content',
        header: 'CONTEXT',
        format: (r) => String((r as UserMentionInfo).content ?? '').slice(0, 60),
      },
      {
        key: 'attachedTo',
        header: 'MESSAGE',
        format: (r) => C.id(String((r as UserMentionInfo).attachedTo ?? '').slice(-12)),
      },
