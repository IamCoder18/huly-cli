    ...
    table(
      ...
      [
        ...
        { key: '_id', header: '_ID', format: (r) => C.id(String((r as SpaceDoc)._id).slice(-16)) },
        {
          ...
          header: 'PRIVATE',
          align: 'center',
          format: (r) => ((r as SpaceDoc).private ? C.warn('yes') : C.muted('—')),
        },
        {
          key: 'archived',
          header: 'ARCHIVED',
          align: 'center',
          format: (r) => ((r as SpaceDoc).archived ? C.muted('yes') : C.ok('—')),
        },
        {
          key: 'members',
          header: 'MEMBERS',
          align: 'right',
          format: (r) => String(((r as SpaceDoc).members ?? []).length),
        },
      ],
...
export async function updateSpace(
  ...
  try {
    ...
    await withSpinner(
      ...
      () =>
        client.updateDoc(
          CLASS.Space as Ref<Class<SpaceDoc>>,
...
export async function listSpaceTypes(
  ...
  try {
    const docs = (await withSpinner(
      'Loading space types…',
      () => client.findAll(CLASS.SpaceType as Ref<Class<SpaceType>>, {}),
      opts,
    )) as SpaceType[]
    if (shouldJson({ json: opts.json, ci: opts.ci })) {
      json(docs)
    ...
    table(
      ...
      [
        { key: 'name', header: 'NAME' },
        {
          ...
          header: 'DESCRIPTION',
          format: (r) => String((r as SpaceType).shortDescription ?? '').slice(0, 50),
        },
...
export async function listSpacePermissions(
  ref: string,
  opts: { json?: boolean; ci?: boolean; workspace?: string; url?: string } = {},
): Promise<void> {
  const client = await connectCli({ url: opts.url, workspace: opts.workspace })
  try {
    ...
    const perms = (await client.findAll(CLASS.Permission as Ref<Class<Permission>>, {
      objectId: spaceId,
    })) as Permission[]
    ...
    const roles =
      roleIds.length > 0
        ? ((await client.findAll('core:class:Role' as Ref<Class<Role>>, {
            _id: { $in: roleIds as Ref<Doc>[] },
          })) as Role[])
        : []
      ...
      [
        {
          ...
          header: 'ROLE',
          format: (r) =>
            C.emphasis(roleMap.get(String((r as Permission).role)) ?? String((r as Permission).role)),
        },
...
export async function addSpaceMembers(
  ...
  try {
    ...
    if (added.length === 0) {
      console.log(C.muted('(no new members)'))
      return
    }
    await withSpinner(
      'Adding members…',
      () =>
        client.updateDoc(
          ...
          'core:space:Space' as Ref<Space>,
          id as Ref<Doc>,
          { $push: { members: { $each: added } } } as any,
        ),
      opts,
    )
...
export async function setSpaceOwners(
  ref: string,
  members: string[],
  opts: { json?: boolean; ci?: boolean; workspace?: string; url?: string },
): Promise<void> {
...
export async function listAssociations(opts: ListAssociationsOpts = {}): Promise<void> {
  ...
  try {
    ...
    if (opts.a)
      q.a = await resolveRef(opts.a, {
        client,
        classId: (opts.aClass ?? 'core:class:Doc') as Ref<Class<Doc>>,
      })
    if (opts.b)
      q.b = await resolveRef(opts.b, {
        client,
        classId: (opts.bClass ?? 'core:class:Doc') as Ref<Class<Doc>>,
      })
    if (opts.aClass) q.aClass = opts.aClass
    if (opts.bClass) q.bClass = opts.bClass
    const docs = (await withSpinner(
      'Loading associations…',
      () => client.findAll(CLASS.Association as Ref<Class<Association>>, q as any),
      opts,
    )) as Association[]
    if (shouldJson({ json: opts.json, ci: opts.ci })) {
      json(docs)
    ...
    if (docs.length === 0) {
      console.log(C.muted('(no associations)'))
      return
    }
    table(
      docs as unknown as Record<string, unknown>[],
      [
        { key: 'a', header: 'A', format: (r) => C.id(String((r as Association).a).slice(-12)) },
        {
          ...
          header: 'A CLASS',
          format: (r) =>
            String((r as Association).aClass)
              .split(':')
              .pop() ?? '',
        },
        { key: 'b', header: 'B', format: (r) => C.id(String((r as Association).b).slice(-12)) },
        {
          ...
          header: 'B CLASS',
          format: (r) =>
            String((r as Association).bClass)
              .split(':')
              .pop() ?? '',
        },
...
export async function createAssociation(opts: CreateAssociationOpts): Promise<void> {
  ...
  try {
    ...
    const data: Record<string, unknown> = {
      ...
      b: bId,
      bClass,
      association: { type: 'Association' as any } as any,
    }
    const id = await withSpinner(
      'Creating association…',
      () =>
        client.addCollection(
          CLASS.Association as Ref<Class<Association>>,
          (aDoc as Doc).space as Ref<Doc>,
          aId,
          aClass,
          'associations',
          data as any,
...
export async function deleteAssociations(
  ...
  try {
    ...
    if (!opts.yes && ids.length > 1)
      throw new CliError(
        ExitCode.Validation,
        `destructive: deleting ${ids.length} associations requires --yes`,
        're-run with --yes',
      )
    const removed: Ref<Doc>[] = []
    const errors: Array<{ id: string; message: string }> = []
    for (const id of ids) {
      const doc = (await client.findOne(CLASS.Association as Ref<Class<Association>>, {
        _id: id as Ref<Association>,
      })) as Association | undefined
      if (!doc) {
...
export async function listRelations(opts: ListRelationsOpts = {}): Promise<void> {
  ...
  try {
    ...
    if (docs.length === 0) {
      console.log(C.muted('(no relations)'))
      return
    }
    table(
      docs as unknown as Record<string, unknown>[],
      [
        {
          key: 'sourceDoc',
          header: 'SOURCE',
          format: (r) => C.id(String((r as Relation).sourceDoc).slice(-12)),
        },
        {
          key: 'targetDoc',
          header: 'TARGET',
          format: (r) => C.id(String((r as Relation).targetDoc).slice(-12)),
        },
...
export async function createRelation(opts: CreateRelationOpts): Promise<void> {
  ...
  try {
    ...
    const id = await withSpinner(
      ...
      () =>
        client.addCollection(
          CLASS.Relation as Ref<Class<Relation>>,
          (sourceDoc as Doc).space as Ref<Doc>,
          sId,
          sClass,
          'relations',
          data as any,
    ...
    if (shouldJson({ json: opts.json, ci: opts.ci })) {
      json({ _id: id, source: sId, target: tId })
      return
    }
...
export async function deleteRelations(
  ...
  try {
    ...
    if (!opts.yes && ids.length > 1)
      throw new CliError(ExitCode.Validation, `destructive: deleting ${ids.length} relations requires --yes`)
    const removed: Ref<Doc>[] = []
    const errors: Array<{ id: string; message: string }> = []
    for (const id of ids) {
      const doc = (await client.findOne(CLASS.Relation as Ref<Class<Relation>>, {
        _id: id as Ref<Relation>,
      })) as Relation | undefined
      if (!doc) {
        errors.push({ id: String(id), message: 'not found' })
        continue
      }
...
export async function listProjectTypes(
  ...
  try {
    const docs = (await withSpinner(
      ...
      () => client.findAll(CLASS.ProjectType as Ref<Class<ProjectType>>, {}),
      opts,
    )) as ProjectType[]
    if (shouldJson({ json: opts.json, ci: opts.ci })) {
      json(docs)
    ...
    table(
      ...
      [
        { key: 'name', header: 'NAME' },
        {
          key: 'shortDescription',
          header: 'DESCRIPTION',
          format: (r) => String((r as ProjectType).shortDescription ?? '').slice(0, 50),
        },
...
export async function getProjectType(
  ...
  try {
    ...
    const doc = await client.findOne(CLASS.ProjectType as Ref<Class<ProjectType>>, {
      _id: id as Ref<ProjectType>,
    })
...
export async function listTaskTypes(opts: ListTaskTypesOpts = {}): Promise<void> {
  ...
  try {
    const q: Record<string, unknown> = {}
    if (opts.projectType)
      q.parent = await resolveRef(opts.projectType, { client, classId: CLASS.ProjectType as Ref<Class<Doc>> })
    const docs = (await withSpinner(
      'Loading task types…',
      () => client.findAll(CLASS.TaskType as Ref<Class<TaskType>>, q as any),
      opts,
    )) as TaskType[]
    if (shouldJson({ json: opts.json, ci: opts.ci })) {
      json(docs)
    ...
    table(
      ...
      [
        { key: 'name', header: 'NAME' },
        {
          key: 'description',
          header: 'DESCRIPTION',
          format: (r) => String((r as TaskType).description ?? '').slice(0, 50),
        },
        {
          key: 'parent',
          header: 'PARENT',
          format: (r) => C.id(String((r as TaskType).parent ?? '').slice(-12)),
        },
...
export async function createTaskType(opts: CreateTaskTypeOpts): Promise<void> {
  if (!opts.projectType || !opts.label)
    throw new CliError(ExitCode.Validation, 'missing --project-type or --label')
  const client = await connectCli({ url: opts.url, workspace: opts.workspace })
  try {
    const ptId = await resolveRef(opts.projectType, { client, classId: CLASS.ProjectType as Ref<Class<Doc>> })
    const pt = await client.findOne(CLASS.ProjectType as Ref<Class<ProjectType>>, {
      _id: ptId as Ref<ProjectType>,
    })
    ...
    const id = await withSpinner(
      ...
      () =>
        client.addCollection(
          CLASS.TaskType as Ref<Class<TaskType>>,
          (pt as Doc).space as Ref<Doc>,
          ptId,
          CLASS.ProjectType as Ref<Class<Doc>>,
          'taskTypes',
          data as any,
        ),
      opts,
...
export async function createIssueStatus(opts: CreateIssueStatusOpts): Promise<void> {
  if (!opts.projectType || !opts.name)
    throw new CliError(ExitCode.Validation, 'missing --project-type or --name')
  const client = await connectCli({ url: opts.url, workspace: opts.workspace })
  try {
    const ptId = await resolveRef(opts.projectType, { client, classId: CLASS.ProjectType as Ref<Class<Doc>> })
    const pt = await client.findOne(CLASS.ProjectType as Ref<Class<ProjectType>>, {
    ...
    if (opts.taskType) {
      taskTypeId = (await resolveRef(opts.taskType, {
        client,
        classId: CLASS.TaskType as Ref<Class<Doc>>,
      })) as Ref<Doc>
    } else {
      ...
      const tts = (await client.findAll(CLASS.TaskType as Ref<Class<TaskType>>, {
        parent: ptId,
      })) as TaskType[]
      if (tts.length === 0)
        throw new CliError(
          ExitCode.NotFound,
          `no task types for project-type ${opts.projectType}; pass --task-type`,
        )
    ...
    const id = await withSpinner(
      'Creating issue status…',
      () =>
        client.addCollection(
          CLASS.IssueStatus as Ref<Class<IssueStatus>>,
          (pt as Doc).space as Ref<Doc>,
          ptId,
          CLASS.ProjectType as Ref<Class<Doc>>,
          'statuses',
