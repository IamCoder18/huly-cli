...
function attachGlobalOpts(cmd: Command, opts: { skipNonInteractive?: boolean } = {}): Command {
  let c = cmd
    ...
    .option(
      '--markdown',
      'output body as markdown (warns and falls back if server cannot convert; --raw-markup on read commands for raw output)',
    )
    .option('--dry-run', 'print intended tx, do not apply')
    .option(
      '--minimal',
      'minimal payload (no smart defaults; equivalent to HULY_OPINIONATED=0 for this invocation)',
    )
...
Examples:
  ...
  ws.command('create')
    .description('Create a new workspace (requires --yes)')
...
Note: workspace creation runs the tracker migration. May take 30-60s.`,
  ...
  ws.command('delete [name]')
    .description('Delete a workspace (DESTRUCTIVE; requires --yes; --force to delete the active workspace)')
...
Examples:
  ...
  wsMember
    .command('add <account>')
    .description("Add or change a member's role (requires OWNER)")
...
Examples:
  ...
  ws.command('info')
    .description('Show current workspace info (name, uuid, region, mode)')
    .addHelpText(
      'after',
      `
Examples:
  ...
  ws.command('guests')
    .description('Update guest settings (--read-only and/or --sign-up, true|false)')
...
For individual guest role assignment, use \`huly workspace member add --role GUEST\`.`,
    ...
    .action(async (opts, cmd) => {
      try {
        const readOnly =
          opts.readOnly === undefined ? undefined : opts.readOnly !== 'false' && opts.readOnly !== '0'
  ...
  ws.command('access-link')
    .description('Create an access link (signup invite) for a role')
...
Examples:
  ...
  user
    .command('get [ref]')
    .description('Show user profile (current user by default, or by ref/uuid)')
...
Examples:
  ...
  user
    .command('find <email>')
    .description('Look up a user by email (account-level or workspace-local)')
    .addHelpText(
      'after',
      `
Examples:
...
Examples:
  ...
  project
    .command('delete <ref...>')
    .description('Delete projects (DESTRUCTIVE; requires --yes)')
    .addHelpText(
      'after',
      `
...
--project <ref>' before deleting.`,
  ...
  project
    .command('statuses [ref]')
    .description('List issue statuses for a project (defaults to $HULY_PROJECT or first arg)')
    .option('--project <ref>')
    .action(async (ref, opts, cmd) => {
      try {
  ...
  project
    ...
    .description('List project target preferences (alias for `target-preference list`)')
    .option('--project <ref>')
    .action(async (opts, cmd) => {
      try {
  ...
  tgtPref
    .command('upsert')
    .description('Create or merge a project target preference')
    .option('--project <ref>')
...
parentTitle on every sub-issue.`,
  ...
  issue
    .command('delete <ref...>')
    .description('Delete issues (requires --yes for multiple)')
    .action(async (refs, opts, cmd) => {
      try {
...
Examples:
    ...
    .action(async (ref, opts, cmd) => {
      try {
        await addIssueRelation(
          ref,
          opts.type as 'blocks' | 'isBlockedBy' | 'relatesTo',
          opts.target,
          globalsFrom(cmd),
        )
      } catch (e) {
  ...
  issue
    .command('move <ref>')
    .description('Move an issue (set/drop parent)')
  ...
  issue
    .command('preview-delete <ref...>')
    .description('Preview the impact of deleting issues')
  ...
  issue
    .command('related-targets')
    .description('List related-issue-targets for a project')
    .option('--project <ref>')
    .action(async (opts, cmd) => {
      try {
  ...
  relatedTarget
    .command('set')
    .description('Create a related-issue-target for a project')
    .option('--project <ref>')
  ...
  tmpl
    .command('add-child <template>')
    .description('Add a child reference to a template')
  ...
  tmpl
    .command('remove-child <template>')
    .description('Remove a child reference from a template')
    .requiredOption('--child <ref>')
    .action(async (template, opts, cmd) => {
      try {
...
and every @-mention in the body is resolved and notified.`,
  ...
  const channel = program
    .command('channel')
    .description('Manage chunter channels (and their messages/threads)')
...
Examples:
  ...
  channel
    ...
    .option(
      '--auto-join',
      'new workspace members join automatically (FORWARD-ONLY; does NOT add existing members)',
    )
...
Examples:
  ...
  channel
    .command('delete <ref...>')
    .description('Delete channels (DESTRUCTIVE; requires --yes)')
    .action(async (refs, opts, cmd) => {
      try {
  ...
  channel
    .command('archive <ref>')
    .description('Archive a channel (--value false to unarchive)')
...
Examples:
  ...
  channel
    .command('join <ref>')
    .description('Join a channel (--member <email> for a specific user)')
...
Examples:
  ...
  channel
    .command('add-member <ref>')
    .description('Add one or more members')
...
Examples:
  ...
  channel
    .command('remove-member <ref>')
    .description('Remove one or more members')
    .requiredOption('--members <email...>', 'space-separated list')
    .addHelpText(
      'after',
      `
...
will not see the message in the channel history until they join.`,
  ...
  dm.command('create')
    .description('Create a DM (with --person or --members)')
    .option('--person <email>')
...
Note: huly dm create does NOT require --yes.`,
  ...
  const dmMsg = dm
    .command('message')
    .description('Manage messages within a DM (alias for `dm messages`/`dm send`)')
  dmMsg
    .command('list <dm>')
    .description('List messages in a DM')
  ...
  dmMsg
    .command('send <dm>')
    .description('Send a DM message (or to <email> via --person)')
    .option('--body <md>')
    .option('--body-file <path>')
...
in the channel sidebar.`,
  ...
  dm.command('messages <dm>')
    .description('[alias] List messages in a DM (use `dm message list`)')
    .action(async (dm, opts, cmd) => {
      try {
        await listDmMessages(dm, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
  ...
  dm.command('send <dm>')
    .description('[alias] Send a DM message (use `dm message send`)')
    .option('--body <md>')
    .option('--body-file <path>')
    .option('--person <email>', 'recipient email (auto-creates DM if needed)')
    .addHelpText(
...
This command is deprecated; use 'huly dm message send <dm> --body ...'.`,
  ...
  thread
    .command('list <target>')
    .description('List replies on a target message')
...
List available tags with \`huly master-tag list\`.`,
  ...
  cardSpace
    .command('create')
    .description('Create a card-space')
    .requiredOption('--name <name>')
    .option('--description <text>')
    .action(async (opts, cmd) => {
      try {
  ...
  cardSpace
    .command('delete <ref...>')
    .description('Delete card-spaces')
    .action(async (refs, opts, cmd) => {
      try {
...
'--owner "Alice Smith"'. Limit 200 results per call.`,
  ...
  action
    ...
    .description('Mark a task done (sets doneOn=now)')
    .addHelpText(
      'after',
      `
...
status past the last Active state (IssueToDoDone mixin).`,
  ...
  action
    .command('reopen <ref>')
    .description('Reopen a task (clears doneOn)')
    .addHelpText(
      'after',
      `
...
manually move the issue back via 'huly issue update --status'.`,
  ...
  action
    .command('schedule <ref>')
    .description('Create a WorkSlot for the task')
...
OnWorkSlotUpdate.`,
  ...
  action
    .command('unschedule <ref>')
    .description('Remove WorkSlots for the task')
...
check 'huly action list --issue <ref>' first.`,
  ...
  const doc = program
    .command('document')
    .description('Manage documents (and their snapshots/inline comments)')
  withGlobalHelp(doc)
  doc
    .command('list')
...
the teamspace by exact title match (lowercased).`,
  ...
  doc
    .command('snapshots <ref>')
    .description('List all snapshots for a document')
    .addHelpText(
      'after',
      `
Examples:
...
Use \`document snapshot --snapshot-id <id>\` to fetch a specific snapshot.`,
  ...
  doc
    ...
    .description('Get a specific snapshot (by --snapshot-id)')
...
N4: \`snapshot\` (singular) gets one; \`snapshots\` (plural) lists all.`,
  ...
  doc
    .command('inline-comments <ref>')
    .description('List inline comments for a document')
    .action(async (ref, opts, cmd) => {
      try {
  ...
  ts.command('delete <ref...>')
    .description('Delete teamspaces')
    .action(async (refs, opts, cmd) => {
      try {
  ...
  cal
    .command('calendars')
    .description('List calendar objects (not events — see `calendar list` for events)')
    .addHelpText(
      'after',
      `
Examples:
...
Examples:
  ...
  cal
    .command('get <ref>')
    .description('Get an EVENT (not a calendar)')
    .option('--raw-markup', 'output raw prosemirror-JSON markup instead of markdown')
    .addHelpText(
      'after',
      `
...
\`calendar calendars --json\` and grep for the calendar id.`,
  ...
  cal
    .command('recurring-instances <ref>')
...
Notes:
  ...
  time
    ...
    .description('Time report for a single issue')
    .addHelpText(
      'after',
      `
...
by OnIssueUpdate).`,
  ...
  const space = program.command('space').description('Manage core Spaces (containers) — Phase 11')
  withGlobalHelp(space)
  space
    .command('list')
    ...
    .action(async (opts, cmd) => {
      try {
        await listSpaces({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  space
    ...
    .action(async (ref, opts, cmd) => {
      try {
        await getSpace(ref, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  space
    ...
    .action(async (ref, opts, cmd) => {
      try {
        await updateSpace(ref, {
          ...opts,
          private: opts.private === undefined ? undefined : !!opts.private,
          archived: opts.archived === undefined ? undefined : !!opts.archived,
          ...globalsFrom(cmd),
        })
      } catch (e) {
        handleError(e)
  ...
  space
    .command('permissions <ref>')
    .description('List permissions on a space')
    .action(async (ref, opts, cmd) => {
      try {
        await listSpacePermissions(ref, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  ...
  space
    ...
    .action(async (ref, opts, cmd) => {
      try {
        await addSpaceMembers(ref, opts.members, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  space
    .command('remove-member <ref>')
    .description('Remove members from a space')
    .requiredOption('--members <email...>')
    .action(async (ref, opts, cmd) => {
      try {
        await removeSpaceMembers(ref, opts.members, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  space
    .command('set-owners <ref>')
    .description('Set owners on a space')
    .requiredOption('--members <email...>')
    .action(async (ref, opts, cmd) => {
      try {
        await setSpaceOwners(ref, opts.members, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  ...
  spaceType
    ...
    .action(async (opts, cmd) => {
      try {
        await listSpaceTypes({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  spaceType
    ...
    .action(async (ref, opts, cmd) => {
      try {
        await getSpaceType(ref, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })

  const assoc = program.command('association').description('Manage Associations (a↔b) — Phase 11')
  withGlobalHelp(assoc)
  assoc
    .command('list')
    ...
    .action(async (opts, cmd) => {
      try {
        await listAssociations({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  assoc
    ...
    .requiredOption('--a <ref>')
    .requiredOption('--b <ref>')
    .option('--a-class <id>')
    .option('--b-class <id>')
    .action(async (opts, cmd) => {
      try {
        await createAssociation({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  assoc
    ...
    .action(async (refs, opts, cmd) => {
      try {
        await deleteAssociations(refs, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })

  const rel = program.command('relation').description('Manage Relations (a→b on parent) — Phase 11')
  withGlobalHelp(rel)
  rel
    .command('list')
    .description('List relations')
    .option('--source <ref>')
    .option('--source-class <id>')
    .option('--target <ref>')
    .action(async (opts, cmd) => {
      try {
        await listRelations({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  rel
    ...
    .requiredOption('--target <ref>')
    .option('--source-class <id>')
    .option('--target-class <id>')
    .option('--name <n>')
    .action(async (opts, cmd) => {
      try {
        await createRelation({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  rel
    ...
    .action(async (refs, opts, cmd) => {
      try {
        await deleteRelations(refs, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  ...
  projectType
    ...
    .action(async (opts, cmd) => {
      try {
        await listProjectTypes({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  projectType
    .command('get <ref>')
    .description('Get a project type')
    .action(async (ref, opts, cmd) => {
      try {
        await getProjectType(ref, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  ...
  taskType
    ...
    .action(async (opts, cmd) => {
      try {
        await listTaskTypes({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  taskType
    ...
    .action(async (opts, cmd) => {
      try {
        await createTaskType({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  ...
  issueStatus
    ...
    .action(async (opts, cmd) => {
      try {
        await createIssueStatus({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  ...
  const activity = program
    .command('activity')
    .description('Manage activity messages, reactions, mentions — Phase 14')
  withGlobalHelp(activity)
  activity
    .command('list')
    .description('List activity messages')
    .option('--target <ref>')
    .option('--target-class <id>')
    .option('--pinned')
    ...
    .action(async (opts, cmd) => {
      try {
        await listActivity({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  activity
    .command('get <ref>')
    .description('Get a single activity message')
    .action(async (ref, opts, cmd) => {
      try {
        await getActivity(ref, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  activity
    .command('pin <ref>')
    .description('Pin an activity message (--unpin to remove)')
    .option('--unpin')
    .action(async (ref, opts, cmd) => {
      try {
        await pinActivity(ref, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  activity
    .command('react')
    .description('Add/remove/list reactions on an activity message')
    .requiredOption('--target <ref>')
  ...
  activity.commands
    ...
    .action(async (target, opts, cmd) => {
      try {
        await listReplies(target, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  activity.commands
    ...
    .action(async (target, opts, cmd) => {
      try {
        await addReply({ ...opts, target, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  activity.commands
    ...
    .action(async (ref, opts, cmd) => {
      try {
        await updateReply(ref, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  activity.commands
    ...
    .action(async (refs, opts, cmd) => {
      try {
        await deleteReplies(refs, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  ...
  activity.commands
    ...
    .action(async (opts, cmd) => {
      try {
        await listSaved({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  activity.commands
    .find((c) => c.name() === 'saved')!
    .command('save')
    .description('Save a message (--target)')
    .requiredOption('--target <ref>')
    .action(async (opts, cmd) => {
      try {
        await saveMessage({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  activity.commands
    .find((c) => c.name() === 'saved')!
    .command('unsave')
    .description('Unsave a message (--target)')
    .requiredOption('--target <ref>')
    .action(async (opts, cmd) => {
      try {
        await unsaveMessage({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  activity
    .command('mentions')
    .description('List @-mentions of the current user')
    .action(async (opts, cmd) => {
      try {
        await listMentions({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  ...
  const notification = program.command('notification').description('Manage inbox notifications — Phase 15')
  withGlobalHelp(notification)
  notification
    .command('list')
    .description('List inbox notifications')
    .option('--read')
    .option('--unread')
    .option('--archived <bool>', '', (v) => v !== 'false' && v !== '0')
    .option('--limit <n>', '', (v) => parseInt(v, 10))
    .action(async (opts, cmd) => {
      try {
        await listInbox({
  ...
  notification
    ...
    .action(async (ref, opts, cmd) => {
      try {
        await getInbox(ref, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  notification
    .command('mark-read <ref...>')
    .description('Mark notifications read')
    .action(async (refs, opts, cmd) => {
      try {
        await markRead(refs, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  notification
    .command('mark-unread <ref...>')
    .description('Mark notifications unread')
    .action(async (refs, opts, cmd) => {
      try {
        await markUnread(refs, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  notification
    ...
    .action(async (opts, cmd) => {
      try {
        await markAllRead({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  notification
    ...
    .action(async (refs, opts, cmd) => {
      try {
        await archive(refs, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  notification
    .command('unarchive <ref...>')
    .description('Unarchive notifications')
    .action(async (refs, opts, cmd) => {
      try {
        await unarchive(refs, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  notification
    ...
    .action(async (opts, cmd) => {
      try {
        await archiveAll({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  notification
    ...
    .action(async (refs, opts, cmd) => {
      try {
        await deleteInbox(refs, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  notification
    ...
    .description('Print unread inbox count')
    .action(async (opts, cmd) => {
      try {
        await unreadCount({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  notification
    ...
    .action(async (opts, cmd) => {
      try {
        await listProviders({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  notification
    ...
    .action(async (opts, cmd) => {
      try {
        await listTypes({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  ...
  notification.commands
    ...
    .action(async (opts, cmd) => {
      try {
        await listContexts({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  notification.commands
    ...
    .action(async (ref, opts, cmd) => {
      try {
        await getContext(ref, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  notification.commands
    .find((c) => c.name() === 'contexts')!
    .command('pin <ref>')
    .description('Pin a context (--unpin to remove)')
    .option('--unpin')
    .action(async (ref, opts, cmd) => {
      try {
        await pinContext(ref, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  notification.commands
    .find((c) => c.name() === 'contexts')!
    .command('hide <ref>')
    .description('Hide a context (--unhide)')
    .option('--unhide')
    .action(async (ref, opts, cmd) => {
      try {
        await hideContext(ref, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  notification
    .command('subscribe')
    .description('Subscribe to notifications on a target')
    .requiredOption('--target <ref>')
    .option('--target-class <id>')
    .action(async (opts, cmd) => {
      try {
        await subscribe({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  notification
    .command('unsubscribe')
    .description('Unsubscribe from notifications on a target')
    .requiredOption('--target <ref>')
    .option('--target-class <id>')
    .action(async (opts, cmd) => {
      try {
        await unsubscribe({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  ...
  notification.commands
    ...
    .action(async (opts, cmd) => {
      try {
        await listSettings({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  notification.commands
    ...
    .requiredOption('--provider <ref>')
    .requiredOption('--type <ref>')
    .requiredOption('--enabled <bool>', 'true|false', (v) => v !== 'false' && v !== '0')
    .action(async (opts, cmd) => {
      try {
        await updateSetting({ ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  ...
  const approval = program.command('approval').description('Manage approval requests — Phase 16')
  withGlobalHelp(approval)
  approval
    .command('list')
    ...
    .action(async (opts, cmd) => {
      try {
        await listApprovals({ ...opts, status: opts.status as any, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  approval
    ...
    .action(async (ref, opts, cmd) => {
      try {
        await getApproval(ref, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  approval
    .command('request')
    .description('Create an approval request on a target')
    ...
    .action(async (opts, cmd) => {
      try {
        await createApproval({ ...opts, txJson: opts.tx, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  approval
    ...
    .action(async (ref, opts, cmd) => {
      try {
        await commentOnApproval({ ...opts, ref, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  approval
    ...
    .action(async (ref, opts, cmd) => {
      try {
        await approveRequest({ ...opts, ref, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  approval
    .command('reject <ref>')
    .description('Reject an approval request (requires --comment)')
    .requiredOption('--comment <md>')
    .option('--rejected-tx <json>', 'JSON-encoded rollback tx to apply on rejection')
    .action(async (ref, opts, cmd) => {
      try {
        await rejectRequest({ ...opts, ref, rejectedTxJson: opts.rejectedTx, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  approval
    .command('cancel <ref>')
    .description('Cancel an approval request (requester only)')
    .action(async (ref, opts, cmd) => {
      try {
        await cancelRequest({ ...opts, ref, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
  approval
    ...
    .action(async (refs, opts, cmd) => {
      try {
        await deleteApprovals(refs, { ...opts, ...globalsFrom(cmd) })
      } catch (e) {
        handleError(e)
      }
    })
