...
- `card-space`: literal `card:space:Default` — **this usually does not exist.** If you don't have a card space, the create will fail with `PLATFORM_NOT_FOUND`. Pass `--card-space <name>` explicitly.
