...
| `huly issue create` / `huly issue update --priority`                | 5 default `TypeIssuePriority` records (`Urgent`, `High`, `Medium`, `Low`, `NoPriority`) in `core:space:Model`   | Workspace has zero `TypeIssuePriority` records AND opinionated defaults are ON. Self-heals workspaces created with `INIT_REPO_DIR=/no-init-scripts` on a self-hosted install (HULY-7). Suppress with `--minimal` or `HULY_OPINIONATED=0`.                                                                       |
...
| `huly issue create`             | `--priority`          | `Medium` if it exists in the workspace; else the top-ranked priority; else omitted. Aliases: `--priority Normal` and `--priority None` resolve to `Medium` and `NoPriority` respectively. If the workspace has zero `TypeIssuePriority` records AND opinionated defaults are ON, the 5 platform priorities are auto-seeded into `core:space:Model` before resolution (HULY-7). Suppress the auto-seed with `--minimal` or `HULY_OPINIONATED=0`. `--dry-run` never persists. |
