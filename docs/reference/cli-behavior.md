...
| `huly issue create`             | `--priority`          | `Medium` if it exists in the workspace; else first priority; else omitted. Aliases: `--priority Normal` and `--priority None` resolve to `Medium` and `NoPriority` respectively. If the workspace has zero `TypeIssuePriority` records AND opinionated defaults are ON, the 5 platform priorities are auto-seeded into `core:space:Model` before resolution (HULY-7). Suppress the auto-seed with `--minimal` or `HULY_OPINIONATED=0`. |
