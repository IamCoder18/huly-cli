...
|---|-------|----------|----------------|------------|------------------------|
| 3.1 | **Model load fixable by `docker compose down -v` + recreate.** Verified by wiping all volumes and reinitializing: all classes (Project, Issue, IssueStatus, Milestone, Component, IssueTemplate, Document, Teamspace, Channel, ChatMessage, Calendar, ToDo, TimeSpendReport (after §2 fix), WorkSlot, Card, MasterTag, CardSpace, Person) all return data correctly from `findAll`. The previous bug was workspace-state corruption from partially-applied upgrade txes from old workspaces; a fresh workspace starts clean. | (fixed) | n/a | n/a | n/a |
| 3.1b | **`WS_OPERATION=all+backup` is in place** (per commit + the comment in `compose.yml`) and the worker correctly idles for workspaces that are at the current version. When the kafka queue replays deletion events for hard-deleted workspaces, the worker briefly spins but settles. **Operational improvement**: deletion backlog no longer wedges the worker. | n/a | (this is the fix, not the bug) | n/a | n/a |
