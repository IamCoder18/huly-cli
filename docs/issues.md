...
|---|-------|----------|----------------|------------|------------------------|
| 3.1 | **Model storage and live-query layer drift.** Transactor's model storage has ~4080 classes loaded, but the SDK's local model state is a 3-key stub. `client.findAll('tracker:class:Component', ...)` returns 0 even after the SDK itself creates the doc, even with the doc present in postgres (verified by direct SQL). The transactor's `doCleanup` and `pendingUpgradeSql` paths run fine (verified with `WS_OPERATION=all+backup`), but the live-query layer never picks up docs the SDK created because the local model is too small. | High | `tracker:class:Issue`, `tracker:class:Milestone`, `tracker:class:Component`, `tracker:class:IssueTemplate`, `document:class:Document`, `document:class:Teamspace`, `calendar:class:Event`, `time:class:ToDo`, `time:class:WorkSlot` all return 0 from `findAll`. `time:class:TimeSpendReport` is `domain not found` entirely. **~30% of the CLI surface.** | None — the SDK's `getModel()` is a stub; re-connecting / re-creating the workspace pod does not change it. | **Yes** — this is in transactor source or in how the workspace pod propagates model state to clients. Not fixable from compose. |
| 3.1b | **`WS_OPERATION=all+backup` is in place** (per commit + the comment in `compose.yml`) and the worker correctly idles for workspaces that are at the current version (`life` at 0.7.423). When the kafka queue replays deletion events for hard-deleted workspaces, the worker briefly spins but settles. **Operational improvement**: deletion backlog no longer wedges the worker. | n/a | (this is the fix, not the bug) | n/a | n/a |
